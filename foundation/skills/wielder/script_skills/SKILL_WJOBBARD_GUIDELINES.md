...
- For cross-cloud jobs, also apply [Provisioning Guidelines](../ops_skills/SKILL_PROVISIONING_GUIDELINES.md): the ecosystem provisioner must wire provider facts, validate completeness, and expose the runtime auth/path/command contract in `show` and `plan`.
