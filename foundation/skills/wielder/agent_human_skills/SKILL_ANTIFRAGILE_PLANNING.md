...
Architectural approval is a boundary, not a mood. A plan may move from planning
to execution only when the operator has explicitly approved the current scope,
sequence, file targets, and validation gates. Do not preserve ceremonial
approval rituals as separate doctrine; keep the approval signal concrete,
auditable, and tied to the plan artifact.
