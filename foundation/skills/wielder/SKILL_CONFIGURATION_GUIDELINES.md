...
* **App Incorporation Cross-Reference**: When copying or reviving an app from another repo, use [App Incorporation](SKILL_APP_INCORPORATION.md) before moving config. Incorporation work tends to expose misplaced project buckets, local wrapper bloat, fixture pressure in defaults, and stale legacy vocabulary.
...
- **Runtime Polymorphism Exception:** A polymorphic app may choose a small runtime implementation branch after canonical resolution when the branch is a real runtime phenotype, not a second loader. Example: an MSA service can resolve one `mmseqs` config contract, then select in-kubernetes vs host-local service target, CPU vs GPU workload profile, or mounted-bucket path behavior from strict config leaves. This is valid only when every selectable shape is declared in HOCON, type-validated at the app boundary, visible in `-w plan`, and does not manually read extra files or reconstruct ecosystem layers.
