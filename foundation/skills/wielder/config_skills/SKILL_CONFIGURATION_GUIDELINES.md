...
* **Versioned Storage Contracts**: Project-owned reusable artifacts and key prefixes should carry a strict versioned storage contract using the canonical `storage_role` vocabulary from Strict FS Agnosticism. Keep `storage_role`, `location`, `storage_tier`, `availability`, and `replication` as independent leaves. Unknown roles must fail typed validation; `availability = global` means Nabu-addressable, not implicitly replicated.
